...
export default async function Home() {
  ...
  contentData.forEach((d) => {
    if (!d.extra) topData.push(d);
    else bottomData.push(d);
  });
    ...
    <>
      <PageHeader>
        <div className="flex">
          ...
          <div className="mt-auto mb-4">
            ...
            <TimeUpdate
              published={metaData.published.toDate()}
              className="text-grayLink mt-2 text-sm sm:text-md"
            />
            {/* <p className="text-grayLink mt-2 text-sm sm:text-md">Last updated {lastUpdated}.</p> */}
            <span className="text-grayLink italic text-xs sm:text-sm">
              {metaData.status}
            </span>
          </div>
        </div>
      ...
      <article className="w-5/6 md:w-3/5 px-8 mt-4 mx-auto">
        <div className="flex flex-col gap-4 text-lg">
          {metaData.introParagraph.map((p) => (
            <p key={uuidv4()}>{p}</p>
          ))}
        </div>

        <h2 className="mt-16 text-xl mini:text-2xl font-medium">
          {metaData.topText}
        </h2>
        ...
        {bottomData.length > 0 && (
          <>
            <h2 className="mt-32 text-xl mini:text-2xl font-medium">
              {metaData.bottomText}
            </h2>
